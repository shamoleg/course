...
Мастер (Master). Мастер ROS обеспечивает регистрацию имени и поиск остальной части графа вычислений. Без мастера
узлы не смогут находить друг друга, обмениваться сообщениями  и вызывать службы.

Мастер ROS работает как служба имен в графе вычислений.
Мастер хранит информацию о регистрации тем и сервисов для узлов  ROS. Узлы общаются с мастером для передачи ему регистрационной информации. Также мастер сообщает узлам о других зарегист-
...
| launch              | Эта папка содержит файлы с расщирением(.launch) для запуска одного или нескольких узлов(ROS nods)
